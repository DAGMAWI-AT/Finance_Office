...
const { UserReportsCollection } = require("../model/report"); // Adjust path based on your structure
   ...
   const getUserReport = async (req, res) => {
    ...
    const getReportByRegistrationId = async (req, res) => {
      ...
      try {
        ...
        const result = await UserReportsCollection.find({ registrationId }).toArray();
    
        // Check if reports are found
        if (result.length === 0) {
          return res.status(404).json({ success: false, message: "No reports found for the given registrationId" });
        }
